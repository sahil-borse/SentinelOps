...
_SPEC: dict[type, tuple[str, dict[str, str]]] = {
    ...
    Notification: ("notifications", {"sent_at": "datetime",
                    "read_at": "datetime"}),
    ScheduledAudit: ("scheduled_audits", {"scope": "json", "planned_date": "date",
                     "conducted_date": "date", "report_generated_at": "datetime",
    ...
    Evidence: ("evidence", {"submitted_at": "datetime", "is_remediation": "bool",
               "document_date": "date"}),
    InboundSubmission: ("inbound_submissions", {"submitted_at": "datetime",
                        "is_remediation": "bool", "document_date": "date"}),
    ...
    GapCategory: ("gap_categories", {"derived_at": "datetime",
                  "merged_from": "json"}),
...
def repositories(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "audits": Repository(conn, ScheduledAudit),
        "notifications": Repository(conn, Notification),
        "categories": Repository(conn, GapCategory),
        "instances": Repository(conn, CheckInstance),
        "evidence": WriteOnceRepository(conn, Evidence),
        # `inbound` is the pre-binding inbox; `rounds` is section 4's review
        # loop. The v2 key `submissions` is deliberately not reused for either:
        # it meant the first and would silently have been read as the second.
        "inbound": Repository(conn, InboundSubmission),
        "rounds": Repository(conn, EvidenceSubmission),
